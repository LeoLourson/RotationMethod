...
void rotate_matrix(int i, int j, double fi1, double fi2, int n, double** a) {
	for (int k=0; k<n; k++) {
		rotate(i, j, fi1, fi2, a[k]); // первый индекс - столбец!!
	}
}

pair<double, double> angle(double x, double y) {
	double r = sqrt(x*x+y*y);
	if (abs(r)< EPS) throw MyException("functions.cpp::fi: division by zero error\n", EC_ZERO);
	double fi1 = x/r;
	double fi2 = y/r;
	return pair<double, double>(fi1, fi2);
}
